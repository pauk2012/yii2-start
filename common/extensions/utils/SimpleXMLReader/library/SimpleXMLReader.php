...
namespace common\extensions\utils\SimpleXMLReader\library;
...
class SimpleXMLReader extends \XMLReader
{
     ...
     */
    public function registerCallback($xpath, $callback, $nodeType = self::ELEMENT)
    {
     ...
     * @return SimpleXMLReader
     */
    public function unRegisterCallback($xpath, $nodeType = self::ELEMENT)
    {
        if (!isset($this->callback[$nodeType][$xpath])) {
    ...
    public function parse()
    {
        ...
        while ($continue && $this->read()) {
            ...
            if (isset($this->callback[$this->nodeType]['*'])) {
                $continue = call_user_func($this->callback[$this->nodeType]['*'], $this);
            } else if (isset($this->callback[$this->nodeType][$this->name])) {
